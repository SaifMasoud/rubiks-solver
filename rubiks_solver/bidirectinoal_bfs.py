...
def bidirectional_bfs_search(start, end, neighbours_func, reverse_edge_func):
    """ neighbours_func should return a list of tuples [(neighbour, edge)].
        reverse_edge_func should return the reverse of an edge (so we can reverse when joining final path)
        """
    # Setup
    print(f"Start: {start}")
    if start == end:
    ...
    move_before_reverse = {end: None}  # Same but for the opposite end of the search.
    ...
    for i in range(GODS_NUMBER//2):
        while True:
            ...
            for next_state, edge in neighbours_func(cur_state):
                ...
                if next_state in other_moves:
                    first_path = _path(start, next_state, move_before, reverse_edge_func)
                    second_path = _path(end, next_state, move_before_reverse, reverse_edge_func)
                    second_path.reverse()
                    return first_path + [reverse_edge_func(e) for e in second_path]
...
if __name__ == "__main__":
    nei_fun = lambda node: [
        (perm_apply(twist, node), twist) for twist in rubik.quarter_twists
    ]
